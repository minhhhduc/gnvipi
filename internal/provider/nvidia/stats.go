...
// object per line. Override with GNVIPI_STATS_FILE.
func StatsLogFile() string {
	if p := os.Getenv("GNVIPI_STATS_FILE"); p != "" {
		return p
	}
...
// readFrame returns frame n (1-based, frame 1 = NEWEST entries) and the total
// frame count. Events inside a frame come back newest-first.
func (l *statsLog) readFrame(n int) ([]RequestEvent, int, error) {
	...
	hi := len(ls) - (n-1)*frameSize
	lo := hi - frameSize
	if lo < 0 {
		lo = 0
	}
	out := make([]RequestEvent, 0, hi-lo)
	for i := hi - 1; i >= lo; i-- { // newest-first
		var ev RequestEvent
		if err := json.Unmarshal(ls[i], &ev); err != nil {
			l.fail(err)
			continue
		}
		out = append(out, ev)
